...
class DATNetFModel(Abstract):
    ...
    def _build_model(self):
        ...
        def compute_src_logits(w_emb, c_emb):
            ...
            rnn_feats = bi_rnn(emb, self.src_seq_len, self.training, self.cfg.num_units, self.cfg.rnn_drop_rate,
        ...
        def compute_tgt_logits(w_emb, c_emb):
            ...
            emb = tf.layers.dropout(tf.concat([w_emb, char_cnn], axis=-1), rate=self.cfg.emb_drop_rate,
                                    training=self.training)
            rnn_feats = bi_rnn(emb, self.tgt_seq_len, self.training, self.cfg.num_units, self.cfg.rnn_drop_rate,
                               activation=tf.tanh, concat=self.cfg.concat_rnn, name="bi_rnn")
    ...
    def train(self, src_dataset, tgt_dataset):
        ...
        for epoch in range(1, self.cfg.epochs + 1):
            ...
            score = self.evaluate_data(tgt_dataset.dev_batches(), name="target_dev", resource="target")
            self.evaluate_data(tgt_dataset.test_batches(), name="target_test", resource="target")
            if score > best_score:
